...
public OnPluginStart()
{
	...
	AutoExecConfig_SetFile("AdvancedRules");
...
public Action:Timer_DisplayRulesToClient(Handle:hTimer, UserId)
{
	...
	PrintToChat(client, "\x01 Please take some time and view the server's rules.");
...
public Action:Timer_DisplayInfoToClient(Handle:hTimer, UserId)
{
	...
	PrintToChat(client, "\x01 You need to view the\x03 !rules\x01 and accept them.");
...
public LoadConfigFile()
{
	...
	do
	{
		...
		KvGetString(keyValues, "name", RuleArray.enRuleName, sizeof(enRules));
		KvGetString(keyValues, "description", RuleArray.enRuleDesc, sizeof(enRules));
		
		ReplaceString(RuleArray.enRuleName, sizeof(enRules), "//q", "\"");
		ReplaceString(RuleArray.enRuleDesc, sizeof(enRules), "//q", "\"");
		...
		PushArrayArray(Array_Rules, RuleArray, sizeof(enRules));
...
ShowClientRule(client, item)
{
	...
	for(new i=0;i < lines;i++)
	{
		_RTLify(RuleDescLines[i], RuleDescLines[i]);
...
{
	new const Float:RTL_minimum = 0.1;
	new rtledWords = 0;
	...
	{
		if (WordAnalysis(tokens[word]) >= RTL_minimum)
		{
		...
		else
		{
			...
			while (WordAnalysis(tokens[lastWord]) < RTL_minimum)
			{
				lastWord++;
			}
